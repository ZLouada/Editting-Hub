...
const appCss = "/assets/styles-DD98b57C.css";
...
const $$splitComponentImporter$a = () => import("./settings-BVwEhMBN.mjs");
...
const $$splitComponentImporter$9 = () => import("./reset-password-C1Im9AWu.mjs");
...
const Route$9 = createFileRoute("/reset-password")({
  ...
  head: () => ({
    meta: [{
      ...
      name: "description",
      content: "Enter your verification code and set a new password for your Editing Hub account."
    }]
  })
});
const $$splitComponentImporter$8 = () => import("./register-D2Ko9xA2.mjs");
const Route$8 = createFileRoute("/register")({
  ...
  head: () => ({
    meta: [{
      ...
      name: "description",
      content: "Create your Editing Hub account to discover and hire top video editors."
    }]
  })
});
const $$splitComponentImporter$7 = () => import("./login-Bjhg0r6z.mjs");
const Route$7 = createFileRoute("/login")({
  ...
  head: () => ({
    meta: [{
      ...
      name: "description",
      content: "Sign in to your Editing Hub account to manage your profile and connect with creators."
    }]
  })
});
const $$splitComponentImporter$6 = () => import("./insights-d2li15gD.mjs");
...
const $$splitComponentImporter$5 = () => import("./forgot-password-CTAEfr4n.mjs");
const Route$5 = createFileRoute("/forgot-password")({
  ...
  head: () => ({
    meta: [{
      ...
      name: "description",
      content: "Reset your Editing Hub password. We'll send a verification code to your email."
    }]
  })
});
const $$splitComponentImporter$4 = () => import("./editors-CnGbG32W.mjs");
...
const $$splitComponentImporter$3 = () => import("./contact-D2GIyEc9.mjs");
...
const $$splitComponentImporter$2 = () => import("./app-C60IhdUK.mjs");
...
const $$splitComponentImporter$1 = () => import("./index-BDZb8YqZ.mjs");
...
function createSupabaseClient() {
  const SUPABASE_URL = "https://pcxvjvcccpvaxiuzucju.supabase.co";
  const SUPABASE_PUBLISHABLE_KEY = "sb_publishable_of4DzbxjW3M_HFERUtc59A_4Yk2rV1Y";
...
async function fetchEditors() {
  const { data, error } = await supabase.from("editors").select("*").order("created_at", { ascending: true });
  if (error) {
...
async function searchEditorsFromDb(query, specialty) {
...
async function fetchEditor(id) {
  const { data: editorRow, error } = await supabase.from("editors").select("*").eq("id", id).single();
  if (error || !editorRow) {
    console.error("[editors] fetchEditor failed:", error?.message);
    const fallback = FALLBACK_EDITORS.find((e) => e.id === id);
    return fallback ? { ...fallback, testimonials: FALLBACK_REVIEWS } : void 0;
  }
  const { data: reviews } = await supabase.from("editor_reviews").select("*").eq("editor_id", id).order("created_at", { ascending: false });
  const editor = mapRow(editorRow);
  editor.testimonials = reviews && reviews.length > 0 ? reviews.map(mapReview) : FALLBACK_REVIEWS;
  return editor;
}
...
const $$splitComponentImporter = () => import("./editors._id-D6BVfQ6j.mjs");
